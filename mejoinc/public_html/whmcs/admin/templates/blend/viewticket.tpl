...
function expandRelServices() {
    $("#relatedservicesexpand").html('<img src="images/loading.gif" align="top" /> {/literal}{$_ADMINLANG.global.loading}{literal}');
...
$("#replyfrm").submit(function () {
	...
	if (response == "true") {
		...
		if (confirm("{/literal}{$_ADMINLANG.support.statuschanged}\n\n{$_ADMINLANG.support.stillsubmit}{literal}")) {
	        return true;
	    }
	    return false;
	}
...
<div class="ticketstaffnotes">
    ...
    <div>
        {$note.message}
        <div style="float:right;"><a href="#" onClick="doDeleteNote('{$note.id}');return false"><img src="images/delete.gif" alt="{$_ADMINLANG.support.deleteticketnote}" border="0" align="absmiddle" /></a></div>
    </div>
</div>
{/foreach}
{/if}
