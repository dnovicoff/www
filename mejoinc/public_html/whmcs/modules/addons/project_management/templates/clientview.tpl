...
{foreach from=$tasks key=tasknum item=task}
        <tr>
            ...
            <td>{if $task.completed}<span class="label active">{$_lang.completed}</span>{/if} {$task.task}{if $task.duein} <span class="taskdue">Due in {$task.duein}Days, {$task.duedate}</span>{/if}{if $task.times}
