...
$lotQuery 		= $DB->Execute("SELECT DISTINCT * FROM lots WHERE park_id='". $parkID ."' ORDER BY lot");
$waterRateQuery	= $DB->Execute("SELECT rate,cutoff,lower_threshold,upper_threshold FROM water_rates WHERE park_id='" . $parkID . "'");
$sewerRateQuery	= $DB->Execute("SELECT rate,cutoff,lower_threshold,upper_threshold FROM sewer_rates WHERE park_id='" . $parkID . "'");
...
$card->SetCreator('Birch Reality, Inc.');
 ...
 while (!$lotQuery->EOF) {
	...
	$usageQuery	= $DB->Execute("SELECT * FROM park_usage WHERE park_id='" . $park->_id . "' AND lot_id='" . $lot->_id . "' AND record_date='" . $recDate . "'");

	
	//	Later values
	$waterBill	= _waterQuailRun($usageQuery->fields['usage'],$waterRateQuery->fields['rate']);
	$sewerBill	= _sewerQuailRun($usageQuery->fields['usage'],$sewerRateQuery->fields['rate']);
	$balance	= $lot->currentBalance() + $waterBill + $sewerBill + $park->_feeAdmin;
	...
	if (($lot->_address != '') && ($lot->_city != '') && ($lot->_zip != '')) {
		$card->Cell(35,3,$lot->_tenant,0,1);
		$card->Cell(35,3,$lot->_address,0,0);
		$card->Cell(37,3,$park->_name,0,1);
		$card->Cell(35,3,$lot->_city . ', ' . $lot->_state . ' ' . $lot->_zip,0,0);
	} else {
		$card->Cell(35,3,$park->_name,0,1);
		$card->Cell(35,3,$park->_address,0,0);
		$card->Cell(37,3,$lot->_tenant,0,1,'R');
		$card->Cell(35,3,$park->_city . ', ' . $park->_state . ' ' . $park->_zip,0,0);
	}
	
	$card->Cell(37,3,'Bill Date: ' . $usageQuery->fields['bill_date'],0,2);
	$card->Cell(37,3,'Due Date: ' . $usageQuery->fields['due_date'],0,1);
	...
	$card->SetFont('Times', '', 6);
	//	First Row
	$card->Cell(13,3,$usageQuery->fields['record_date'],0,0);
	$card->Cell(13,3,$usageQuery->fields['usage'],0,0);		// CHANGE TO - Current months "meter read"
	$card->Cell(13,3,$usageQuery->fields['usage'],0,0);		// CHANGE TO - "previous meter read" - "current meter read"
	$card->Cell(20,3,'Lot Rent',0,0,'L');
	$card->Cell(13,3,$lot->_feeRent,0,0,'R');
	$card->Cell(4,3,'',0,0);	// Spacer
	$card->Cell(13,3,$lot->_lotID,0,0,'C');
	$card->Cell(13,3,$usageQuery->fields['due_date'],0,1,'C');
	
	//	Second Row
	$card->Cell(13,3,$usageQuery->fields['record_date'],0,0);	// CHANGE TO - Previous months record date
	$card->Cell(26,3,$usageQuery->fields['usage'],0,0);		// CHANGE TO - Previous months "meter read"
 ...
 $filename	= $park->_name . ' - Card List - ' . $usageQuery->fields['record_date'];
