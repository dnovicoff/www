				...
				<div id="portfolio">
					...
					<?php else: //If it is a single portfolio item ?>		
						<div class="default-page">
							<?php $siteaddress = get_post_meta($post->ID, '_siteaddress', 1); ?>
							<h2 class="cufon-plain"><a href="<?php echo $siteaddress; ?>"><?php the_title(); ?></a></h2>
							<?php $description = get_post_meta($post->ID, '_description', 1); ?>
							<?php if ($description): ?>
								<h4 class="portfolio-description cufon-plain"><?php echo $description; ?></h4>
							<?php endif; ?>
							<?php 
								$portfolio_image_medium = get_post_meta($post->ID, '_portfolio_image_medium', 1);
