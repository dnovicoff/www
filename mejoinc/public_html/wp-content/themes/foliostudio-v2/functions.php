...
$contact_page_panel = new ECF_Panel('contact-page-image', 'Contact Page Details', 'page', 'side', '');
...
$contact_page_panel->add_fields(array(
	...
	ECF_Field::factory('textarea', 'location', 'Google Embed Code')->set_description('410 x 310'),
));
...
$portfolio_panel->add_fields(array(
	$portfolio_image,
	$portfolio_featured,
	ECF_Field::factory('text', 'siteaddress', 'Site Address'),
	ECF_Field::factory('textarea', 'description', 'Description'),
));
...
function f_print_navigation()  	{
	...
	$admin_login = str_replace ( '/wp-content/themes/' . get_template(), '/wp-login.php', get_bloginfo ( 'template_url' ) );
	?>
    <script type="text/javascript" charset="utf-8">
    	jQuery(function($) {
			
    		<?php if ( get_option('enable_login_form') == 'yes' ) :	?>
				$('#navigation ul:last-child').append( '<li class="login"><a href="<?php echo $admin_login; ?>">Login</a></li>');
				
				$('#navigation ul ul li.login').remove();
	    		//Login form
	    		$('#navigation ul li.login').append('<div class="login-dd dd"><form action="<?php echo get_option('home'); ?>/wp-login.php" method="post" name="loginform"><div class="form-holder"><div class="login-field"><input type="text" name="log" class="field blink" value="Username or email" title="Username or email" /></div><div class="login-field"><input type="password" class="field blink" value="Password" title="Password" name="pwd" /></div><input type="submit" value="Login" class="login-button" /></div></form></div>');
		    <?php else: ?>
...
/* ------- Large Button Shortcode --------*/

function large_button_shortcode( $atts, $content = null )
{
	extract( shortcode_atts( array(
      ...
      return '<p><a class="space-large-button" href="'. $link .'">
			
			<div class="mojo_' . $align . '">
			
			<button class="mojo_button_' . $color . '">
			
			<span class="mojo_button_'. $color .'">
			
			<em class="mojo_button_'. $color .'">' . $content . '</em>
...
add_shortcode('large-button', 'large_button_shortcode');

/* ------- Small Button Shortcode --------*/

function small_button_shortcode( $atts, $content = null )
{
	extract( shortcode_atts( array(
      'link' => '',
	  'align' => '',
      ...
      return '<p><a href="'. $link .'">
			
			<div class="mojo_' . $align . '">
			
			<button class="mojo_button_small">
			
			<span class="mojo_button_small">
			
			<em class="mojo_button_small">' . $content . '</em>
...
add_shortcode('small-button', 'small_button_shortcode');

/* ------- Image Shortcode --------*/

function image_border($atts, $content=null, $code="") {  

	$return = '<div class="mojo-image">';
...
add_shortcode('image' , 'image_border' );
