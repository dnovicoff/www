...
class wp_option_image extends wp_option_file {
	...
	function validate_file() {
		...
		if ($this->max_dimensions) {
			if ($valid[0] > $this->max_dimensions[0]) {
				$this->validation_error = "The uploaded image exceeds the maximum width of " . $this->max_dimensions[0] . "px.";
				return INVALID_VALUE;
			}
			if ($valid[1] > $this->max_dimensions[1]) {
				$this->validation_error = "The uploaded image exceeds the maximum height of " . $this->max_dimensions[1] . "px.";
				return INVALID_VALUE;
			}
		}
		return VALID_VALUE;
	}
	function set_max_dimensions($max_dims) {
		$this->max_dimensions = $max_dims;
	}
}
